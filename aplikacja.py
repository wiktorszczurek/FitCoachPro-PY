...
class Photo(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(100), nullable=False)
    data = db.Column(db.String(500), nullable=False)  # zmienione na String
...
def delete_user(user_id):
    ...
    if user:
        # Usuń wszystkie wiadomości z modelu Message powiązane z użytkownikiem
        messages = Message.query.filter_by(user_id=user.id).all()
        for message in messages:
            db.session.delete(message)

        # Usuń wszystkie wiadomości z modelu ChatMessage wysłane przez użytkownika
        sent_messages = ChatMessage.query.filter_by(sender_id=user.id).all()
        for message in sent_messages:
            db.session.delete(message)

        # Usuń wszystkie wiadomości z modelu ChatMessage otrzymane przez użytkownika
...
def register():
    ...
    if form.validate_on_submit():
        ...
        # generowanie tokena do potwierdzenia e-maila
        token = generate_confirmation_token(new_user.email)

        confirm_url = url_for("confirm_email", token=token, _external=True)

        # wysłanie wiadomości e-mail do użytkownika z linkiem do potwierdzenia
        msg = MailMessage(
...
def add_plan(user_id):
    ...
    # Pobieranie wszystkich ćwiczeń, planów treningowych użytkownika i zdjęć
    ...
    for plan in user_plans:
        with open(plan.filepath, "r", encoding="utf-8") as file:
            try:
                ...
                # Jeśli odczyt w kodowaniu 'utf-8' nie powiedzie się, spróbuj kodowania 'latin-1'
                file.seek(0)
                content = file.read(encoding="latin-1")
        if hasattr(plan, "photo"):  # Sprawdź, czy 'plan' ma atrybut 'photo'
            photo = Photo.query.filter_by(name=plan.photo).first()
        else:
            photo = None  # Lub dowolne domyślne zdjęcie, które chcesz użyć
        plans_and_contents.append({"plan": plan, "content": content, "photo": photo})

    photos = current_user.photos  # Ta linijka została zmieniona
    return render_template(
        "training_plans.html",
...
def add_photo():
    if request.method == "POST":
        ...
        note = request.form.get("note")  # notatka jest opcjonalna

        # bezpiecznie zapisujemy plik i pobieramy jego nazwę
        ...
        # tworzymy nowy obiekt Photo i zapisujemy go w bazie danych
        # zauważ, że zapisujemy tylko nazwę pliku jako data, a nie całą ścieżkę
        new_photo = Photo(name=name, data=filename, note=note)
        db.session.add(new_photo)
        db.session.commit()

        # po zapisaniu zdjęcia przekierowujemy użytkownika z powrotem do strony dodawania zdjęć
        return redirect(url_for("add_photo"))

    # jeśli metoda to GET, to pobieramy wszystkie zdjęcia i wyświetlamy je
...
def delete_photo(id):
    ...
    if photo:
        ...
        if os.path.isfile(file_path):  # sprawdzamy, czy plik istnieje
            os.remove(file_path)  # usuwamy plik
            db.session.delete(photo)  # usuwamy informacje o pliku z bazy danych
            db.session.commit()
    ...
    if request.method == "POST":
        ...
        if not content:
            ...
            mail_message = MailMessage(
                ...
                sender="wiktor.szczurek1@gmail.com",  # Tu powinien być Twój adres e-mail
                recipients=[user.email],
...
def delete_message(message_id):
    ...
    user_id = (
        message.user_id
    )  # zapamiętaj id użytkownika, do którego przypisana była wiadomość
    ...
    )  # przekieruj z powrotem do strony wiadomości użytkownika
...
def messages():
    ...
    # Zaktualizuj status wszystkich wiadomości dla tego użytkownika jako przeczytane
...
def admin_chat(user_id):
    ...
    if request.method == "POST":
        ...
        if not content:
            ...
            # Zaktualizuj status wszystkich wiadomości od tego użytkownika jako przeczytane
...
def support():
    if request.method == "POST":
        ...
        # Tworzenie treści wiadomości e-mail na podstawie wybranej kategorii
...
def add_article():
    if not current_user.admin:
        abort(403)  # Brak dostępu - administrator wymagany
